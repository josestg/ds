...
func NewDynamicArray[T any](capacity int) *DynamicArray[T] {
	if capacity <= 0 {
		panic("ds.NewDynamicArray: must have at minimum 1 capacity")
	}
	return &DynamicArray[T]{
		backend: NewArray[T](capacity),
		size:    0,
	}
}
...
func (d *DynamicArray[T]) grow() {
	...
	newBackend := NewArray[T](newCapacity)
...
func (d *DynamicArray[T]) Remove(index int) T {
	...
	d.backend.Set(d.size-1, zeroValueOf[T]())
	d.size--
	return val
}
...
func (d *DynamicArray[T]) checkBounds(index int) {
	if index < 0 || index >= d.Size() {
		panic("ds.DynamicArray: index out of range")
	}
}
