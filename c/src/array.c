...
status_t array_init(array_t *self, const size_t length,
...
array_t *array_new(size_t length, size_t elem_size) {
  ...
  if (array_init(self, length, elem_size) != S_OK) {
    free(self);
    return NULL;
  }
...
status_t array_set(const array_t *self, const size_t index,
                   const void *value_in, const size_t value_size) {
  if (self == NULL)
    return S_ERR_SELF_IS_NULL;
...
status_t array_get(const array_t *self, const size_t index, void *value_out,
