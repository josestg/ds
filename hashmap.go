...
type HashMap[K comparable, V any] struct {
	size          int
	buckets       []*SinglyLinkedList[*Entry[K, V]]
	hashFunction  func(K) int
	loadThreshold float64
}

type HashMapOptions[K comparable] struct {
	Capacity      int
	LoadThreshold float64
...
func NewHashMap[K comparable, V any]() *HashMap[K, V] {
	return NewHashMapWith[K, V](HashMapOptions[K]{
...
func NewHashMapWith[K comparable, V any](opts HashMapOptions[K]) *HashMap[K, V] {
	capacity := cmp.Or(opts.Capacity, DefaultCapacity)
	loadFactor := cmp.Or(opts.LoadThreshold, DefaultLoadThreshold)
	if loadFactor <= 0 || loadFactor >= 1 {
		panic("HashMap.NewHashMapWith: load factor must be in range (0,1) exclusive")
	}
	...
	buckets := make([]*SinglyLinkedList[*Entry[K, V]], capacity)
	for i := range buckets {
		buckets[i] = NewSinglyLinkedList[*Entry[K, V]]()
	}
...
func (h *HashMap[K, V]) Del(key K) {
	...
	for i, v := range seq.Enum(entries.Iter) {
		if v.key == key {
			_ = entries.Remove(i)
			h.size--
			break
...
func (h *HashMap[K, V]) growAndRehash() {
	h2 := NewHashMapWith[K, V](HashMapOptions[K]{
