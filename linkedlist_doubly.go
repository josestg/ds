...
import "github.com/josestg/ds/seq"
...
func (l *DoublyLinkedList[E]) Get(index int) E {
	...
	l.checkBounds(index)
	data, ok := seq.ValueAt(l.Iter, index)
	if !ok {
...
func (l *DoublyLinkedList[E]) Set(index int, data E) {
	...
	l.checkBounds(index)
	n, ok := seq.ValueAt(l.iterForward, index)
	if !ok {
		panic("DoublyLinkedList.Get: should be unreachable")
	}
	n.data = data
